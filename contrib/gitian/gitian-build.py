...
gsigs = 'https://github.com/mask-project/gitian.sigs.git'
...
def setup():
    ...
    if not os.path.isdir('mask'):
        subprocess.check_call(['git', 'clone', args.url, 'mask'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/mask/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'mask='+args.commit, '--url', 'mask='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/mask-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/mask/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/mask/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/mask-project/mask', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/mask', exist_ok=True)
    os.chdir('builder/inputs/mask')
    if args.pull:
