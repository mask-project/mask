...
used by Mask. The protocol is largely inherited from cryptonote, but has
undergone some changes.

This document also may differ from the `struct bucket_head2` in Mask's
...
> This document does not currently list all data being sent by the mask
> protocol, that portion is a work-in-progress. Please take the time to do it
> if interested in learning about Mask p2p traffic!


## Header
This header is sent for every Mask p2p message.
...
Mask when the link is not encrypted. SSL has been proposed as a means to
...
### Command
An unsigned 32-bit little endian integer representing the Mask specific
...
### Requests
Requests are the basis of the admin protocol for Mask. The `Q` bit must be
