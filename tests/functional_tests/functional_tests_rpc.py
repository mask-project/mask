...
maskd_base = [builddir + "/bin/maskd", "--regtest", "--fixed-difficulty", str(DIFFICULTY), "--offline", "--no-igd", "--p2p-bind-port", "maskd_p2p_port", "--rpc-bind-port", "maskd_rpc_port", "--zmq-rpc-bind-port", "maskd_zmq_port", "--non-interactive", "--disable-dns-checkpoints", "--check-updates", "disabled", "--rpc-ssl", "disabled", "--log-level", "1"]
maskd_extra = [
  [],
  ["--rpc-payment-address", "44SKxxLQw929wRF6BA9paQ1EWFshNnKhXM3qz6Mo3JGDE2YG3xyzVutMStEicxbQGRfrYvAAYxH6Fe8rnD56EaNwUiqhcwR", "--rpc-payment-difficulty", str(DIFFICULTY), "--rpc-payment-credits", "5000", "--data-dir", builddir + "/functional-tests-directory/maskd1"],
]
wallet_base = [builddir + "/bin/mask-wallet-rpc", "--wallet-dir", WALLET_DIRECTORY, "--rpc-bind-port", "wallet_port", "--disable-rpc-login", "--rpc-ssl", "disabled", "--daemon-ssl", "disabled", "--daemon-port", "18180", "--log-level", "1"]
wallet_extra = [
]
...
for i in range(N_MASKDS):
  command_lines.append([str(18180+i) if x == "maskd_rpc_port" else str(18280+i) if x == "maskd_p2p_port" else str(18380+i) if x == "maskd_zmq_port" else x for x in maskd_base])
  if i < len(maskd_extra):
    command_lines[-1] += maskd_extra[i]
  outputs.append(open(builddir + '/tests/functional_tests/maskd' + str(i) + '.log', 'a+'))
