...
Core tests take longer than any other Mask tests, due to the high amount of computational work involved in validating core components.
...
To run only Mask's core tests (after building):
...
To run only Mask's crypto tests (after building):
...
```bash
maskd --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `mask-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Mask's hash tests (after building):
...
To run only Mask's performance tests (after building):
...
To run only Mask's unit tests (after building):
